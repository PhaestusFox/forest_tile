...
fn fragment(mesh: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let cell_w = view.viewport.z / f32(cell_size.x);
    let cell_h = view.viewport.w / f32(cell_size.y);
    var x = (i32(mesh.uv.x) % i32(cell_size.x));
    var y = (i32(mesh.uv.y) % i32(cell_size.y));

    // let cell_x = floor(mesh.offset.x);
    // let cell_y = floor(mesh.offset.y);
...
fn vertex(vertex: Vertex) -> VertexOutput {
    ...
    var x = (i32(out.uv.x) % i32(cell_size.x));
    var y = (i32(out.uv.y) % i32(cell_size.y));
    out.offset = vec2(
        f32(x),
        f32(y),
    );
